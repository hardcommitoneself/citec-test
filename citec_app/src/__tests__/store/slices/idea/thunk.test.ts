...
import { getIdeas } from 'store/slices/ideas/thunk'
...
describe('test in thunk static data', () => {
   ...
   test('should return correct data in getIdeas()', async () => {
      const ideaResponse = [
         {
            portfolio_id: 50,
            portfolio: 'bla bla',
            benchmark: 'SPY',
            creation_date: '2022-07-05 17:08:07',
            ticker: ['ISRG'],
            outstanding_balance: [55714.04],
      ...
      mock
         .onGet('/citec/portfolio/all/', {
            params: {
               detail: 'true',
            },
         })
         .reply(200, ideaResponse)

      const response = await getIdeas()

      expect(response).toEqual(ideaResponse)
   })
})
